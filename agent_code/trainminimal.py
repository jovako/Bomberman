...
for i in range(1,9):
    file=open(f"minimal{i}/data.pickle", "rb")
    while True:
        try:
            x=pickle.load(file)
            counter+=1
...
class data_generator(Sequence):
    ...
    def __init__(self, first=1,batches=batches,batch_size=batch_size):
        ...
        self.file=open(f"minimal{first}/data.pickle", "rb")
        self.num=1  
    ...
    def __getitem__(self,idx):
        ...
        for i in range(self.batch_size):
            try:
                ...
                if self.num>7:
                    self.num=1
                self.file=open(f"minimal{self.num}/data.pickle", "rb")
                x,y=pickle.load(self.file) 
            xs[i]=x
            ys[i]=y
        return xs,ys
...
validation=data_generator(first=8,batches=batches//8)
