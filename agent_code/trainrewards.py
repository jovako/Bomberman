...
#from tensorflow.keras.losses import CosineSimilarity as cos
import argparse
...
class data_generator(Sequence):
    ...
    def __init__(self, first=1,batches=batches,batch_size=batch_size):
        ...
        self.file=open(f"rewards{first}/data.pickle", "rb")
        self.num=1  
    ...
    def __getitem__(self,idx):
        ...
        for i in range(self.batch_size):
            try:
                ...
                if self.num>7:
                    self.num=1
                self.file=open(f"rewards{self.num}/data.pickle", "rb")
                x,y=pickle.load(self.file) 
